...
test("GIVEN the hesitation rule option is enabled, THEN it shows the selected state", async () => {
...
test("GIVEN the hesitation rule option is disabled, THEN it shows the unselected state", async () => {
  renderComponent(
    <HesitationRuleOption isEnabled={false} onToggle={vi.fn()} />,
  );
...
test("GIVEN the hesitation rule option is clicked, THEN it calls the callback", async () => {
  ...
  const { user } = renderComponent(
    <HesitationRuleOption isEnabled={false} onToggle={onToggle} />,
  );

...
const createStorage = (storedValue: string | null = null, legacyValue: string | null = null) => {
...
const resetHook = (storedValue: string | null = null, legacyValue: string | null = null) => {
...
test('GIVEN the hook is created, WHEN a reward is added, THEN it increments the total and persists it', () => {
  ...
  expect(storage.setItem).toHaveBeenCalledWith('multiplication-app', expect.any(String));
});
...
test('GIVEN rewards are added more than once, WHEN the total is updated repeatedly, THEN the persisted total accumulates each reward', () => {
  ...
  expect(storage.setItem).toHaveBeenLastCalledWith('multiplication-app', expect.any(String));
});

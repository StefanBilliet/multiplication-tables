...
export const practiceScreenPage = (user: UserEvent) => ({
  ...
  answerOption: (answer: number | string) => screen.getByRole('button', { name: String(answer) }),
  backToTablesButton: () => screen.getByRole('button', { name: /back to tables/i }),
  checkAnswerButton: () => screen.getByRole('button', { name: /check answer/i }),
  ...
  completePartialSession: async (questionCount: number, selectedTable: number) => {
